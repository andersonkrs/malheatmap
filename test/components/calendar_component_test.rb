...
class CalendarComponentTest < ViewComponent::TestCase
  def group_activities(dates:, activities: {})
    dates.index_with { |date| activities.fetch(date, 0) }
  end
  ...
  test "renders days names correctly" do
    ...
    component
      .css(".calendar > .days > .day")
      .then { |elements| assert_equal %w[Sun Mon Tue Wed Thu Fri Sat], elements.map(&:text) }
  end
  ...
  test "renders each month correctly" do
    ...
    component
      .css(".calendar > .months > .month")
      .then { |elements| assert_equal %w[May Jun Jul Aug Sep Oct Nov Dec Jan Feb Mar Apr May], elements.map(&:text) }
  end
