...
class Subscription::ProcessableTest < ActiveSupport::TestCase
  ...
  test "processing should broadcast the form with the errors returned by the crawler" do
    ...
    form = render(partial: "subscriptions/form", locals: { subscription: Subscription.new(process_errors: ["error!"]) })
  ...
  test "processing should broadcast internal server redirect when something unexpected happens" do
    ...
    broadcast_content =
      turbo_stream_action_tag("redirect", :target => "body", :path => internal_error_path, "turbo-action" => "replace")
