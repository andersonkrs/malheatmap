...
class User::CrawlableTest < ActiveSupport::TestCase
  setup do
    @user = users(:babyoda)
    @crawler_response = {
      profile: {
  ...
  test "raises an error if the oldest crawled entry is older than 30 days" do
    ...
    assert_raises User::Crawlable::CrawlingLogEntry::DeletingOldHistoryNotAllowed do
      @user.crawl_data
    end
  ...
  test "schedules deactivation for non linked account when retuning a profile not found error" do
    ...
    assert_match(/Profile not found/, @user.errors[:base].first)
